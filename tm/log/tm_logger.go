...
func NewTMLogger(w io.Writer) log.Logger {
...
func NewTMLoggerWithColorFn(w io.Writer, colorFn func(keyvals ...interface{}) term.FgBgColor) log.Logger {
	return &tmLogger{term.NewLogger(w, log.NewTMFmtLogger, colorFn)}
}
...
func (l *tmLogger) Error(msg string, keyvals ...interface{}) {
	...
	if err := lWithMsg.Log(keyvals...); err != nil {
		if len(keyvals) == 1 {
			lWithMsg.Log("err", keyvals[0])
		} else {
			lWithMsg.Log("err", err)
			fmt.Println("[ERROR] ", keyvals)
		}
	}
}
